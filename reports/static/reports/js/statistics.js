    ...
    function renderChart(chartType, data) {
        ...
        switch (data.analysis_type) {
            case 'grouped':
                ...
                chartData = { labels: data.results.map(item => item.group), datasets: [{ label: 'Value', data: data.results.map(item => item.value), backgroundColor: colors }] };
                break;
            case 'bivariate': case 'regression':
                ...
                chartData = { datasets: [{ type: 'scatter', label: `${data.parameters.y_field} vs ${data.parameters.x_field}`, data: scatterData, backgroundColor: 'rgba(0, 123, 255, 0.6)' }] };
                if (data.analysis_type === 'regression') {
                    ...
                    chartData.datasets.push({ type: 'line', label: 'Regression Line', data: [{ x: xMin, y: slope * xMin + intercept }, { x: xMax, y: slope * xMax + intercept }], borderColor: '#F44336', fill: false, tension: 0.1 });
                }
                break;
            case 'kmeans':
                ...
                chartData = { datasets: [] };
                for (let i = 0; i < k; i++) { chartData.datasets.push({ type: 'scatter', label: `Cluster ${i}`, data: data.results.filter(p => p.cluster === i), backgroundColor: colors[i % colors.length] }); }
                break;
        }
